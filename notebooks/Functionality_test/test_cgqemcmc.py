...
def run_visual_tests():
    ...
    chains = Parallel(n_jobs=-1)(
        delayed(run_chain_with_seed)(seed, runner, proposer=proposer, n_hops=steps, initial_state=random_initial_states[iter], verbose=False) for iter, seed in enumerate(np.arange(0, reps))
    )  # np.random.randint(0, 2**31 - 1, size=reps)
    ...
    cg = CoarseGraining(n=n, subgroups=subgroups, subgroup_probs=np.ones(len_subgroups) / len_subgroups)

    cg_proposer = QeProposal(model=model, gamma=(0.3, 0.6), time=(1, 20), coarse_graining=cg)

    print("\nRunning Coarse-Grained manual QeMCMC...")
    cg_chains = Parallel(n_jobs=-1)(
        delayed(run_chain_with_seed)(seed, runner, proposer=cg_proposer, n_hops=steps, initial_state=random_initial_states[iter], verbose=False) for iter, seed in enumerate(np.arange(0, reps))
    )  # np.random.randint(0, 2**31 - 1, size=reps)
    ...
    cg_2_chains = Parallel(n_jobs=-1)(
        delayed(run_chain_with_seed)(seed, runner, proposer=cg_proposal_2, n_hops=steps, initial_state=random_initial_states[iter], verbose=False) for iter, seed in enumerate(np.arange(0, reps))
    )
    ...
    loc_chains = Parallel(n_jobs=-1)(
        delayed(run_chain_with_seed)(seed, runner, proposer=loc_proposer, n_hops=steps, initial_state=random_initial_states[iter], verbose=False) for iter, seed in enumerate(np.arange(0, reps))
    )
    ...
    uni_chains = Parallel(n_jobs=-1)(
        delayed(run_chain_with_seed)(seed, runner, proposer=uni_proposer, n_hops=steps, initial_state=random_initial_states[iter], verbose=False) for iter, seed in enumerate(np.arange(0, reps))
    )
