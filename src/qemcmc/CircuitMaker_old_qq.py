...
class CircuitMaker:
    ...
    def __init__(
        ...
        self.pm = qiskit.transpiler.generate_preset_pass_manager(
            optimization_level=0, basis_gates=fixed_basis_gates
        )
    ...
    def get_state_obtained_binary(self, s: str) -> str:
        ...
        if type(s) is not str:
            raise TypeError(
                "s must be a string in get_state_obtained_binary in CircuitMaker"
            )
    ...
    def fn_qc_h1(self) -> QuantumCircuit:
        ...
        for j in range(0, self.n_spins):
            ...
            # print("evo_gate definition:", evo_gate.definition)

            # do this to avoid transpiling in qulacs backend
            # evo_gate = evo_gate.to_matrix()
            # qc_h1.unitary(evo_gate, [self.n_spins-1-j])

            qc_h1.append(evo_gate.definition, [self.n_spins - 1 - j])
